...
mutable struct Optimizer <: MOI.AbstractOptimizer
    inner::Union{FilterSqpProblem,Nothing}
...
function Optimizer(; kwargs...)
    if length(kwargs) > 0
        ...
        `filterSQP.Optimizer` is deprecated. Use
...
MOI.get(::Optimizer, ::MOI.SolverName) = "filterSQP"
...
# In setting up the data for filterSQP, we order the constraints as follows:
...
function constraint_types(model::Optimizer)
    cstype = ""
    cstype *= repeat("L", length(model.linear_le_constraints))
    cstype *= repeat("L", length(model.linear_ge_constraints))
    cstype *= repeat("L", length(model.linear_eq_constraints))
    cstype *= repeat("N", length(model.quadratic_le_constraints))
    cstype *= repeat("N", length(model.quadratic_ge_constraints))
    cstype *= repeat("N", length(model.quadratic_eq_constraints))
    cstype *= repeat("N", length(model.nlp_data.constraint_bounds))
    return cstype
end
...
function MOI.optimize!(model::Optimizer)
    ...
    if model.silent
        addOption(model.inner, "iprint", 0)
    end
...
function MOI.get(model::Optimizer, ::MOI.TerminationStatus)
    ...
    elseif status == :Crash_in_user_supplied_routines
        return MOI.INVALID_MODEL
    elseif status == :Unexpected_ifail_from_QP_solver
        return MOI.OTHER_ERROR
    elseif status == :Not_enough_REAL_workspace_or_parameter_error
        return MOI.MEMORY_LIMIT
    elseif status == :Not_enough_INTEGER_workspace_or_parameter_error
        return MOI.MEMORY_LIMIT
    else
        error("Unrecognized filterSQP status $status")
    end
end
...
macro define_constraint_primal(function_type, set_type, prefix)
    ...
    quote
        function MOI.get(
            ...
            return model.inner.c[ci.value+$offset_function(model)]
        end
    end
end
